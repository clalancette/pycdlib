...
class DirectoryRecordDate(InterfaceISODate):
    ...
    def parse(self, datestr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("Directory Record Date already initialized")
    ...
    def new(self, tm=None):
        ...
        if tm is not None:
            raise pycdlibexception.PyIsoException("Directory Record Date does not support passing tm in")
    ...
    def record(self):
        ...
        if not self.initialized:
            raise pycdlibexception.PyIsoException("Directory Record Date not initialized")
...
class VolumeDescriptorDate(InterfaceISODate):
    ...
    def parse(self, datestr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("This Volume Descriptor Date object is already initialized")

        if len(datestr) != 17:
            raise pycdlibexception.PyIsoException("Invalid ISO9660 date string")
    ...
    def record(self):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("This Volume Descriptor Date is not yet initialized")
    ...
    def new(self, tm=None):
        ...
        if tm is not None:
            ...
            self.hundredthsofsecond = 0
            self.gmtoffset = gmtoffset_from_tm(tm, local)
            self.date_str = "%s%s%s" % (time.strftime(self.time_fmt, local), "{:0<2}".format(self.hundredthsofsecond), struct.pack("=b", self.gmtoffset))
            self.present = True
        else:
            self.year = 0
            self.month = 0
