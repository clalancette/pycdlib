...
class RRSPRecord(object):
    ...
    def parse(self, rrstr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("SP record already initialized!")
        ...
        if su_len != RRSPRecord.length():
            raise pycdlibexception.PyIsoException("Invalid length on rock ridge extension")
        if check_byte1 != 0xbe or check_byte2 != 0xef:
            raise pycdlibexception.PyIsoException("Invalid check bytes on rock ridge extension")
    ...
    def record(self):
        ...
        if not self.initialized:
            raise pycdlibexception.PyIsoException("SP record not yet initialized!")
...
class RRRRRecord(object):
    ...
    def parse(self, rrstr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("RR record already initialized!")
    ...
    def append_field(self, fieldname):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("RR record not yet initialized!")
        ...
        if fieldname == "PX":
            ...
            raise pycdlibexception.PyIsoException("Unknown RR field name %s" % (fieldname))
...
class RRCERecord(object):
    ...
    def parse(self, rrstr, rr_version):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("CE record already initialized!")
    ...
    def record(self):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("CE record not yet initialized!")
...
class RRPXRecord(object):
    ...
    def record(self, rr_version):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("PX record not yet initialized!")
        ...
        if rr_version == "1.09":
            ...
            raise pycdlibexception.PyIsoException("Invalid rr_version")
...
class RRERRecord(object):
    ...
    def parse(self, rrstr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("ER record already initialized!")
    ...
    def record(self):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("ER record not yet initialized!")
...
class RRESRecord(object):
    ...
    def parse(self, rrstr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("ES record already initialized!")
    ...
    def record(self):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("ES record not yet initialized!")
...
class RRPNRecord(object):
    ...
    def parse(self, rrstr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("PN record already initialized!")
    ...
    def record(self):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("PN record not yet initialized!")
...
class RRSLRecord(object):
    ...
    def parse(self, rrstr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("SL record already initialized!")
        ...
        while data_len > 0:
            ...
            if not cr_flags in [0, 1, 2, 4, 8]:
                raise pycdlibexception.PyIsoException("Invalid Rock Ridge symlink flags 0x%x" % (cr_flags))

            if (cr_flags & (1 << 1) or cr_flags & (1 << 2) or cr_flags &(1 << 3)) and len_cp != 0:
                raise pycdlibexception.PyIsoException("Rock Ridge symlinks to dot or dotdot should have zero length")

            if (cr_flags & (1 << 1) or cr_flags & (1 << 2) or cr_flags & (1 << 3)) and name != "":
                raise pycdlibexception.PyIsoException("Cannot have RockRidge symlink that is both a continuation and dot or dotdot")
    ...
    def add_component(self, symlink_comp):
        ...
        if (self.current_length() + 2 + len(symlink_comp)) > 255:
            raise pycdlibexception.PyIsoException("Symlink would be longer than 255")
...
class RRNMRecord(object):
    ...
    def parse(self, rrstr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("NM record already initialized!")
        ...
        if (self.posix_name_flags & 0x7) not in [0, 1, 2, 4]:
            raise pycdlibexception.PyIsoException("Invalid Rock Ridge NM flags")

        if name_len != 0:
            if (self.posix_name_flags & (1 << 1)) or (self.posix_name_flags & (1 << 2)) or (self.posix_name_flags & (1 << 5)):
                raise pycdlibexception.PyIsoException("Invalid name in Rock Ridge NM entry (0x%x %d)" % (self.posix_name_flags, name_len))
    ...
    def record(self):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("NM record not yet initialized!")
...
class RRCLRecord(object):
    ...
    def parse(self, rrstr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("CL record already initialized!")
        ...
        if child_log_block_num_le != utils.swab_32bit(child_log_block_num_be):
            raise pycdlibexception.PyIsoException("Little endian block num does not equal big endian; corrupt ISO")
    ...
    def record(self):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("CL record not yet initialized!")
...
class RRPLRecord(object):
    ...
    def parse(self, rrstr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("PL record already initialized!")
    ...
    def record(self):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("PL record not yet initialized!")
...
class RRTFRecord(object):
    ...
    def parse(self, rrstr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("TF record already initialized!")
        ...
        if su_len < 5:
            raise pycdlibexception.PyIsoException("Not enough bytes in the TF record")
    ...
    def record(self):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("TF record not yet initialized!")
...
class RRSFRecord(object):
    ...
    def parse(self, rrstr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("SF record already initialized!")
    ...
    def record(self):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("SF record not yet initialized!")
...
class RRRERecord(object):
    ...
    def parse(self, rrstr):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("RE record already initialized!")
    ...
    def record(self):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("RE record not yet initialized")
...
class RockRidgeBase(object):
        ...
        while True:
            if left == 0:
                ...
                if record[offset] != '\x00':
                    raise pycdlibexception.PyIsoException("Invalid pad byte")
                break
            elif left < 4:
                raise pycdlibexception.PyIsoException("Not enough bytes left in the System Use field")

            (rtype, su_len, su_entry_version) = struct.unpack("=2sBB", record[offset:offset+4])
            if su_entry_version != SU_ENTRY_VERSION:
                raise pycdlibexception.PyIsoException("Invalid RR version %d!" % su_entry_version)

            if rtype == 'SP':
                if left < 7 or not is_first_dir_record_of_root:
                    raise pycdlibexception.PyIsoException("Invalid SUSP SP record")
                ...
                if self.rr_version is not None and self.rr_version != "1.09":
                    raise pycdlibexception.PyIsoException("RR record not allowd in a non 1.09 Rock Ridge ISO")
                ...
                if self.rr_version is not None and self.rr_version != version:
                    raise pycdlibexception.PyIsoException("PX record doesn't agree with Rock Ridge version")
                ...
                raise pycdlibexception.PyIsoException("Unknown SUSP record %s" % (utils.hexdump(rtype)))
            offset += su_len
            left -= su_len
    ...
    def record(self):
        ...
        '''
        if not self.initialized:
            raise pycdlibexception.PyIsoException("Rock Ridge extension not yet initialized")
...
class RockRidgeContinuation(RockRidgeBase):
    ...
    def extent_location(self):
        ...
        if self.new_extent_loc is None and self.orig_extent_loc is None:
            raise pycdlibexception.PyIsoException("No extent assigned to Rock Ridge Continuation!")
...
class RockRidge(RockRidgeBase):
    ...
    def parse(self, record, is_first_dir_record_of_root, bytes_to_skip):
        ...
        '''
        if self.initialized:
            raise pycdlibexception.PyIsoException("Rock Ridge extension already initialized")
    ...
    def new(self, is_first_dir_record_of_root, rr_name, isdir, symlink_path,
        ...
        if rr_version != "1.09" and rr_version != "1.12":
            raise pycdlibexception.PyIsoException("Only Rock Ridge versions 1.09 and 1.12 are implemented")
    ...
    def add_to_file_links(self):
        ...
        if self.px_record is None:
            if self.ce_record is None:
                raise pycdlibexception.PyIsoException("No Rock Ridge file links and no continuation entry")
    ...
    def update_child_link(self):
        ...
        if self.child_link is None:
            raise pycdlibexception.PyIsoException("No child link found!")
        ...
        if self.cl_record is not None:
            ...
            if self.ce_record is not None and self.ce_record.continuation_entry.cl_record is not None:
                ...
                raise pycdlibexception.PyIsoException("Could not find child link record!")
    ...
    def update_parent_link(self):
        ...
        if self.parent_link is None:
            raise pycdlibexception.PyIsoException("No parent link found!")
        ...
        if self.pl_record is not None:
            ...
            if self.ce_record is not None and self.ce_record.continuation_entry.pl_record is not None:
                ...
                raise pycdlibexception.PyIsoException("Could not find parent link record!")
    ...
    def child_link_block_num(self):
        ...
        raise pycdlibexception.PyIsoException("This RR record has no child link record")
