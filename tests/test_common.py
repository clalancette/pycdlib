...
def internal_check_file(dirrecord, name, dr_len, loc, datalen, hidden):
...
def internal_check_rr_longname(iso, dir_record, extent, letter):
    internal_check_file(dir_record, letter.upper()*8+b".;1", None, extent, 3, hidden=False)
...
def check_onefile(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[2], b"FOO.;1", 40, 24, 4, hidden=False)
...
def check_twofiles(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BAR.;1", 40, 24, 4, hidden=False)
    ...
    # and its contents should be "foo\n".
    internal_check_file(iso.pvd.root_dir_record.children[3], b"FOO.;1", 40, 25, 4, hidden=False)
    internal_check_file_contents(iso, '/FOO.;1', b"foo\n")
...
def check_onefile_onedirwithfile(iso, filesize):
    ...
    internal_check_file(dir1_record.children[2], b"BAR.;1", 40, 26, 4, hidden=False)
...
def check_twoextentfile(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BIGFILE.;1", 44, 24, 2049, hidden=False)
...
def check_twoleveldeepfile(iso, filesize):
    ...
    internal_check_file(subdir1_record.children[2], b"FOO.;1", 40, 26, 4, hidden=False)
...
def check_joliet_onefile(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[2], b"FOO.;1", 40, 30, 4, hidden=False)
    internal_check_file_contents(iso, "/FOO.;1", b"foo\n", 'iso_path')
    ...
    # contents should be "foo\n".
    internal_check_file(iso.joliet_vd.root_dir_record.children[2], "foo".encode('utf-16_be'), 40, 30, 4, hidden=False)
...
def check_joliet_onefileonedir(iso, filesize):
    ...
    # contents should be "foo\n".
    internal_check_file(iso.pvd.root_dir_record.children[3], b"FOO.;1", 40, 32, 4, hidden=False)
    internal_check_file_contents(iso, "/FOO.;1", b"foo\n", 'iso_path')

    # Now check the Joliet file.  It should have a name of "foo", it should have
    # a directory record length of 40, it should start at extent 32, and its
    # contents should be "foo\n".
    internal_check_file(iso.joliet_vd.root_dir_record.children[3], "foo".encode('utf-16_be'), 40, 32, 4, hidden=False)
    internal_check_file_contents(iso, "/foo", b"foo\n", 'joliet_path')
...
def check_eltorito_nofiles(iso, filesize):
    ...
    # extent 25.
    internal_check_file(iso.pvd.root_dir_record.children[3], b"BOOT.CAT;1", 44, 25, 2048, hidden=False)
    ...
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BOOT.;1", 40, 26, 5, hidden=False)
...
def check_eltorito_twofile(iso, filesize):
    ...
    # it should have a directory record length of 44, and it should start at
    # extent 25.
    internal_check_file(iso.pvd.root_dir_record.children[4], b"BOOT.CAT;1", 44, 25, 2048, hidden=False)

    # Now check the boot file.  It should have a name of BOOT.;1, it should
    # have a directory record length of 40, it should start at extent 26, and
    # its contents should be "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[3], b"BOOT.;1", 40, 26, 5, hidden=False)
    internal_check_file_contents(iso, "/BOOT.;1", b"boot\n")
    ...
    # its contents should be "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"AA.;1", 38, 27, 3, hidden=False)
...
def check_rr_onefile(iso, filesize):
    ...
    internal_check_file(foo_dir_record, b"FOO.;1", 116, 25, 4, hidden=False)
    internal_check_file_contents(iso, "/FOO.;1", b"foo\n")
...
def check_rr_twofile(iso, filesize):
    ...
    internal_check_file(bar_dir_record, b"BAR.;1", 116, 25, 4, hidden=False)
    internal_check_file_contents(iso, "/BAR.;1", b"bar\n")
    ...
    internal_check_file(foo_dir_record, b"FOO.;1", 116, 26, 4, hidden=False)
    internal_check_file_contents(iso, "/FOO.;1", b"foo\n")
...
def check_rr_onefileonedirwithfile(iso, filesize):
    ...
    internal_check_file(bar_dir_record, b"BAR.;1", 116, 27, 4, hidden=False)
    internal_check_file_contents(iso, "/DIR1/BAR.;1", b"bar\n")
...
def check_alternating_subdir(iso, filesize):
    ...
    internal_check_file(bb_dir_record, b"BB.;1", 38, 26, 3, hidden=False)
    ...
    internal_check_file(dd_dir_record, b"DD.;1", 38, 27, 3, hidden=False)
    ...
    internal_check_file(sub1_dir_record, b"SUB1.;1", 40, None, 5, hidden=False)
    ...
    internal_check_file(sub2_dir_record, b"SUB2.;1", 40, None, 5, hidden=False)
...
def check_rr_verylongname_joliet(iso, filesize):
    ...
    # a directory record length of 40, it should start at extent 30, and its
    # contents should be "foo\n".
    internal_check_file(iso.joliet_vd.root_dir_record.children[2], ("a"*64).encode('utf-16_be'), 162, 32, 3, hidden=False)
...
def check_joliet_and_rr_onefile(iso, filesize):
    ...
    # its contents should be "foo\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"FOO.;1", 116, 31, 4, hidden=False)
    internal_check_file_contents(iso, '/FOO.;1', b"foo\n")

    # Now check the Joliet file.  It should have a name of "foo", it should have
    # a directory record length of 40, it should start at extent 31, and its
    # contents should be "foo\n".
    internal_check_file(iso.joliet_vd.root_dir_record.children[2], "foo".encode('utf-16_be'), 40, 31, 4, hidden=False)
    internal_check_file_contents(iso, "/foo", b"foo\n", which='joliet_path')
...
def check_rr_and_eltorito_nofiles(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[3], b"BOOT.CAT;1", 124, 26, 2048, hidden=False)

    # Now check the boot file.  It should have a name of BOOT.;1, it should
    # have a directory record length of 116 (for Rock Ridge), it should start
    # at extent 27, and its contents should be "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BOOT.;1", 116, 27, 5, hidden=False)
    internal_check_file_contents(iso, "/BOOT.;1", b"boot\n")
...
def check_rr_and_eltorito_onefile(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[4], b"FOO.;1", 116, 28, 4, hidden=False)
    internal_check_file_contents(iso, '/FOO.;1', b"foo\n")
...
def check_rr_and_eltorito_onedir(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[3], b"BOOT.CAT;1", 124, 27, 2048, hidden=False)

    # Now check the boot file.  It should have a name of BOOT.;1, it should
    # have a directory record length of 116 (for Rock Ridge), it should start
    # at extent 27, and its contents should be "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BOOT.;1", 116, 28, 5, hidden=False)
    internal_check_file_contents(iso, "/BOOT.;1", b"boot\n")
...
def check_joliet_and_eltorito_nofiles(iso, filesize):
    ...
    # it should have a directory record length of 44, and it should start at
    # extent 31.
    internal_check_file(iso.pvd.root_dir_record.children[3], b"BOOT.CAT;1", 44, 31, 2048, hidden=False)

    # Now check the boot file.  It should have a name of BOOT.;1, it should
    # have a directory record length of 40, it should start at extent 32, and
    # its contents should be "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BOOT.;1", 40, 32, 5, hidden=False)
    internal_check_file_contents(iso, "/BOOT.;1", b"boot\n", 'iso_path')

    # Now check the boot catalog file.  It should have a name of BOOT.CAT;1,
    # it should have a directory record length of 44, and it should start at
    # extent 31.
    internal_check_file(iso.joliet_vd.root_dir_record.children[3], "boot.cat".encode('utf-16_be'), 50, 31, 2048, hidden=False)

    internal_check_file(iso.joliet_vd.root_dir_record.children[2], "boot".encode('utf-16_be'), 42, 32, 5, hidden=False)
...
def check_isohybrid(iso, filesize):
    ...
    # it should have a directory record length of 44, and it should start at
    # extent 35.
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BOOT.CAT;1", 44, 25, 2048, hidden=False)
    ...
    # extent 26.
    internal_check_file(iso.pvd.root_dir_record.children[3], b"ISOLINUX.BIN;1", 48, 26, 68, hidden=False)
...
def check_isohybrid_mac_uefi(iso, filesize):
    ...
    # should have a directory record length of 48, and it should start at
    # extent 26.
    internal_check_file(iso.pvd.root_dir_record.children[3], b"EFIBOOT.IMG;1", 46, None, 1, hidden=False)
...
def check_joliet_and_eltorito_onefile(iso, filesize):
    ...
    # should contain "foo\n".
    internal_check_file(iso.pvd.root_dir_record.children[4], b"FOO.;1", 40, 33, 4, hidden=False)
    internal_check_file_contents(iso, '/FOO.;1', b"foo\n", 'iso_path')

    internal_check_file(iso.joliet_vd.root_dir_record.children[2], "boot".encode('utf-16_be'), 42, 32, 5, hidden=False)
    internal_check_file_contents(iso, "/boot", b"boot\n", 'joliet_path')

    internal_check_file(iso.joliet_vd.root_dir_record.children[4], "foo".encode('utf-16_be'), 40, 33, 4, hidden=False)
    internal_check_file_contents(iso, '/foo', b"foo\n", 'joliet_path')
...
def check_joliet_and_eltorito_onedir(iso, filesize):
    ...
    # it should have a directory record length of 44, and it should start at
    # extent 33.
    internal_check_file(iso.pvd.root_dir_record.children[3], b"BOOT.CAT;1", 44, 33, 2048, hidden=False)

    # Now check the boot file.  It should have a name of BOOT.;1, it should have
    # a directory record length of 40, it should start at extent 34, and it
    # should contain "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BOOT.;1", 40, 34, 5, hidden=False)
    internal_check_file_contents(iso, "/BOOT.;1", b"boot\n", 'iso_path')
    ...
    internal_check_file(iso.joliet_vd.root_dir_record.children[3], "boot.cat".encode('utf-16_be'), 50, 33, 2048, hidden=False)

    internal_check_file(iso.joliet_vd.root_dir_record.children[2], "boot".encode('utf-16_be'), 42, 34, 5, hidden=False)
    internal_check_file_contents(iso, "/boot", b"boot\n", 'joliet_path')
...
def check_joliet_rr_and_eltorito_nofiles(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[3], b"BOOT.CAT;1", 124, 32, 2048, hidden=False)

    # Now check the boot file.  It should have a name of BOOT.;1, it should have
    # a directory record length of 116 (for Rock Ridge), it should start at
    # extent 33, and it should contain "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BOOT.;1", 116, 33, 5, hidden=False)
    internal_check_file_contents(iso, "/BOOT.;1", b"boot\n")

    internal_check_file(iso.joliet_vd.root_dir_record.children[3], "boot.cat".encode('utf-16_be'), 50, 32, 2048, hidden=False)

    internal_check_file(iso.joliet_vd.root_dir_record.children[2], "boot".encode('utf-16_be'), 42, 33, 5, hidden=False)
    internal_check_file_contents(iso, "/boot", b"boot\n", which='joliet_path')
...
def check_joliet_rr_and_eltorito_onefile(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[4], b"FOO.;1", 116, 34, 4, hidden=False)
    internal_check_file_contents(iso, '/FOO.;1', b"foo\n")
    ...
    internal_check_file(iso.joliet_vd.root_dir_record.children[4], "foo".encode('utf-16_be'), 40, 34, 4, hidden=False)
    internal_check_file_contents(iso, "/foo", b"foo\n", which='joliet_path')
...
def check_joliet_rr_and_eltorito_onedir(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[3], b"BOOT.CAT;1", 124, 34, 2048, hidden=False)

    # Now check the boot file.  It should have a name of BOOT.;1, it should have
    # a directory record length of 116 (for Rock Ridge), it should start at
    # extent 35, and it should contain "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BOOT.;1", 116, 35, 5, hidden=False)
    internal_check_file_contents(iso, "/BOOT.;1", b"boot\n")
    ...
    internal_check_file(iso.joliet_vd.root_dir_record.children[3], "boot.cat".encode('utf-16_be'), 50, 34, 2048, hidden=False)

    internal_check_file(iso.joliet_vd.root_dir_record.children[2], "boot".encode('utf-16_be'), 42, 35, 5, hidden=False)
    internal_check_file_contents(iso, "/boot", b"boot\n", which='joliet_path')
...
def check_xa_onefile(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[2], b"FOO.;1", 54, 24, 4, hidden=False)
    internal_check_file_contents(iso, "/FOO.;1", b"foo\n")
...
def check_xa_joliet_onefile(iso, filesize):
    ...
    # a directory record length of 54 (for the XA record), it should start at
    # extent 24, and it should contain "foo\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"FOO.;1", 54, 30, 4, hidden=False)
    internal_check_file_contents(iso, "/FOO.;1", b"foo\n", 'iso_path')
...
def check_isolevel4_onefile(iso, filesize):
    ...
    # a directory record length of 54 (for the XA record), it should start at
    # extent 24, and it should contain "foo\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"foo", 36, 25, 4, hidden=False)
    internal_check_file_contents(iso, "/foo", b"foo\n")
...
def check_isolevel4_eltorito(iso, filesize):
    ...
    # it should have a directory record length of 124 (for Rock Ridge), and it
    # should start at extent 34.
    internal_check_file(iso.pvd.root_dir_record.children[3], b"boot.cat", 42, 26, 2048, hidden=False)

    # Now check the boot file.  It should have a name of BOOT.;1, it should have
    # a directory record length of 116 (for Rock Ridge), it should start at
    # extent 35, and it should contain "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"boot", 38, 27, 5, hidden=False)
    internal_check_file_contents(iso, "/boot", b"boot\n")
...
def check_everything(iso, filesize):
    ...
    internal_check_file(boot_rec, b"boot", 128, 50, 5, hidden=False)
    internal_check_file_contents(iso, "/boot", b"boot\n")
    ...
    # it should have a directory record length of 124 (for Rock Ridge), and it
    # should start at extent 34.
    internal_check_file(iso.pvd.root_dir_record.children[3], b"boot.cat", 136, 49, 2048, hidden=False)
    ...
    internal_check_file(dir1_record.children[3], b"foo", 126, 51, 4, hidden=False)
    internal_check_file_contents(iso, "/dir1/foo", b"foo\n")

    # Now check the boot file.  It should have a name of BOOT.;1, it should have
    # a directory record length of 116 (for Rock Ridge), it should start at
    # extent 35, and it should contain "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[5], b"foo", 126, 51, 4, hidden=False)
    internal_check_file_contents(iso, "/foo", b"foo\n")
    ...
    # a directory record length of 116 (for Rock Ridge), it should start at
    # extent 35, and it should contain "boot\n".
    internal_check_file(dir8_record.children[2], b"bar", 126, 52, 4, hidden=False)
...
def check_rr_xa_onefile(iso, filesize):
    ...
    # a directory record length of 54 (for the XA record), it should start at
    # extent 24, and it should contain "foo\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"FOO.;1", 130, 25, 4, hidden=False)
    internal_check_file_contents(iso, "/FOO.;1", b"foo\n")
    internal_check_file_contents(iso, "/foo", b"foo\n", which='rr_path')
...
def check_rr_joliet_symlink(iso, filesize):
    ...
    # its contents should be "foo\n".
    foo_dir_record = iso.pvd.root_dir_record.children[2]
    internal_check_file(foo_dir_record, b"FOO.;1", 116, 31, 4, hidden=False)
    internal_check_file_contents(iso, "/FOO.;1", b"foo\n")
...
def check_eltorito_multi_boot(iso, filesize):
    ...
    # have a directory record length of 40, it should start at extent 26, and
    # its contents should be "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[4], b"boot2", 38, 28, 6, hidden=False)
...
def check_eltorito_multi_boot_hard_link(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[5], b"bootlink", 42, 28, 6, hidden=False)
...
def check_eltorito_boot_info_table(iso, filesize):
    ...
    # its contents should be "boot\n".
    boot_rec = iso.pvd.root_dir_record.children[2]
    internal_check_file(boot_rec, b"boot", 38, 27, 5, hidden=False)
    internal_check_file_contents(iso, "/boot", b"boot\n")
...
def check_eltorito_boot_info_table_large(iso, filesize):
    ...
    # its contents should be "boot\n".
    boot_rec = iso.pvd.root_dir_record.children[2]
    internal_check_file(boot_rec, b"boot", 38, 27, 80, hidden=False)
...
def check_hard_link(iso, filesize):
    ...
    internal_check_file(dir1_record.children[2], b"FOO.;1", 40, 25, 4, hidden=False)
    internal_check_file_contents(iso, "/DIR1/FOO.;1", b"foo\n")
...
def check_joliet_isolevel4(iso, filesize):
    ...
    # directory record length of 40, it should start at extent 32, and its
    # contents should be "foo\n".
    internal_check_file(iso.pvd.root_dir_record.children[3], b"foo", 36, 33, 4, hidden=False)
    internal_check_file_contents(iso, "/foo", b"foo\n")

    # Now check the Joliet file.  It should have a name of "foo", it should have
    # a directory record length of 40, it should start at extent 32, and its
    # contents should be "foo\n".
    internal_check_file(iso.joliet_vd.root_dir_record.children[3], "foo".encode('utf-16_be'), 40, 33, 4, hidden=False)
    internal_check_file_contents(iso, "/foo", b"foo\n")
...
def check_joliet_and_eltorito_nofiles_hide_iso_only(iso, filesize):
    ...
    internal_check_file(iso.joliet_vd.root_dir_record.children[3], "boot.cat".encode('utf-16_be'), 50, None, 2048, hidden=False)
...
def check_hard_link_reshuffle(iso, filesize):
    ...
    # it should have a directory record length of 44, and it should start at
    # extent 25.
    internal_check_file(iso.pvd.root_dir_record.children[3], b"FOO.;1", 40, 24, 4, hidden=False)
    internal_check_file_contents(iso, "/FOO.;1", b"foo\n")
...
def check_eltorito_boot_info_table_large_odd(iso, filesize):
    ...
    # its contents should be "boot\n".
    boot_rec = iso.pvd.root_dir_record.children[2]
    internal_check_file(boot_rec, b"boot", 38, 27, 81, hidden=False)
...
def check_zero_byte_file(iso, filesize):
    ...
    # it should have a directory record length of 44, and it should start at
    # extent 25.
    internal_check_file(iso.pvd.root_dir_record.children[3], b"FOO.;1", 40, 25, 0, hidden=False)
...
def check_eltorito_hd_emul(iso, filesize):
    ...
    # have a directory record length of 40, it should start at extent 26, and
    # its contents should be "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BOOT.;1", 40, 26, 512, hidden=False)
...
def check_eltorito_floppy12(iso, filesize):
    ...
    # have a directory record length of 40, it should start at extent 26, and
    # its contents should be "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BOOT.;1", 40, 26, 1228800, hidden=False)
...
def check_eltorito_floppy144(iso, filesize):
    ...
    # have a directory record length of 40, it should start at extent 26, and
    # its contents should be "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BOOT.;1", 40, 26, 1474560, hidden=False)
...
def check_eltorito_floppy288(iso, filesize):
    ...
    # have a directory record length of 40, it should start at extent 26, and
    # its contents should be "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"BOOT.;1", 40, 26, 2949120, hidden=False)
...
def check_onefile_with_semicolon(iso, filesize):
    ...
    # should have a directory record length of 40, it should start at extent 24,
    # and its contents should be "foo\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"FOO;1.;1", 42, 24, 4, hidden=False)
...
def check_eltorito_rr_verylongname(iso, filesize):
    ...
    # it should have a directory record length of 44, and it should start at
    # extent 25.
    internal_check_file(iso.pvd.root_dir_record.children[2], b"AAAAAAAA.;1", None, 27, 2048, hidden=False)

    # Now check the boot file.  It should have a name of BOOT.;1, it should
    # have a directory record length of 40, it should start at extent 26, and
    # its contents should be "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[3], b"BOOT.;1", 116, 28, 5, hidden=False)
    internal_check_file_contents(iso, "/BOOT.;1", b"boot\n")
...
def check_isohybrid_file_before(iso, filesize):
    ...
    # it should have a directory record length of 44, and it should start at
    # extent 25.
    internal_check_file(iso.pvd.root_dir_record.children[3], b"FOO.;1", 40, 27, 4, hidden=False)
    internal_check_file_contents(iso, "/FOO.;1", b"foo\n")

    # Now check the boot file.  It should have a name of ISOLINUX.BIN;1, it
    # should have a directory record length of 48, and it should start at
    # extent 26.
    internal_check_file(iso.pvd.root_dir_record.children[4], b"ISOLINUX.BIN;1", 48, 26, 68, hidden=False)
...
def check_eltorito_rr_joliet_verylongname(iso, filesize):
    ...
    # it should have a directory record length of 44, and it should start at
    # extent 25.
    internal_check_file(iso.pvd.root_dir_record.children[2], b"AAAAAAAA.;1", None, 33, 2048, hidden=False)

    # Now check the boot file.  It should have a name of BOOT.;1, it should
    # have a directory record length of 40, it should start at extent 26, and
    # its contents should be "boot\n".
    internal_check_file(iso.pvd.root_dir_record.children[3], b"BOOT.;1", 116, 34, 5, hidden=False)
    internal_check_file_contents(iso, "/BOOT.;1", b"boot\n")

    joliet_name = "a"*64
    internal_check_file(iso.joliet_vd.root_dir_record.children[2], joliet_name.encode('utf-16_be'), 162, 33, 2048, hidden=False)

    internal_check_file(iso.joliet_vd.root_dir_record.children[3], "boot".encode('utf-16_be'), 42, 34, 5, hidden=False)
    internal_check_file_contents(iso, "/boot", b"boot\n", which='joliet_path')
...
def check_long_file_name(iso, filesize):
    ...
    # should have a directory record length of 40, it should start at extent 24,
    # and its contents should be "foo\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"FOOBARBAZ1.;1", 46, 24, 11, hidden=False)
...
def check_duplicate_pvd_joliet(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[2], b"FOO.;1", 40, 31, 4, hidden=False)
    internal_check_file_contents(iso, '/FOO.;1', b"foo\n")

    internal_check_file(iso.pvds[1].root_dir_record.children[2], b"FOO.;1", 40, 31, 4, hidden=False)
...
def check_onefile_toolong(iso, filesize):
    ...
    # should have a directory record length of 40, it should start at extent 24,
    # and its contents should be "foo\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"FOO.;1", 40, 24, 2048, hidden=False)
...
def check_joliet_different_names(iso, filesize):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[3], b"FOOJ.;1", 116, 32, 10, hidden=False)
...
def check_rr_onefile_onetwelve(iso, size):
    ...
    internal_check_file(iso.pvd.root_dir_record.children[2], b"FOO.;1", 118, 25, 4, hidden=False)
    internal_check_file_contents(iso, "/FOO.;1", b"foo\n")
    internal_check_file_contents(iso, "/foo", b"foo\n", which='rr_path')
...
def check_joliet_ident_encoding(iso, filesize):
    ...
    # directory record length of 40, it should start at extent 30, and its
    # contents should be "foo\n".
    internal_check_file(iso.pvd.root_dir_record.children[2], b"meta-data", 124, 32, 25, hidden=False)
    internal_check_file(iso.pvd.root_dir_record.children[3], b"user-data", 124, 33, 78, hidden=False)

    # Now check the Joliet file.  It should have a name of "foo", it should have
    # a directory record length of 40, it should start at extent 30, and its
    # contents should be "foo\n".
    internal_check_file(iso.joliet_vd.root_dir_record.children[2], "meta-data".encode('utf-16_be'), 52, 32, 25, hidden=False)
    internal_check_file(iso.joliet_vd.root_dir_record.children[3], "user-data".encode('utf-16_be'), 52, 33, 78, hidden=False)
