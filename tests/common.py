...
def internal_check_dot_dir_record(dot_record, rr=False, rr_nlinks=0):
    ...
    if rr:
        ...
        assert(dot_record.rock_ridge.ce_record.continuation_entry.ext_id == 'RRIP_1991A')
        assert(dot_record.rock_ridge.ce_record.continuation_entry.ext_des == 'THE ROCK RIDGE INTERCHANGE PROTOCOL PROVIDES SUPPORT FOR POSIX FILE SYSTEM SEMANTICS')
        assert(dot_record.rock_ridge.ce_record.continuation_entry.ext_src == 'PLEASE CONTACT DISC PUBLISHER FOR SPECIFICATION SOURCE.  SEE PUBLISHER IDENTIFIER IN PRIMARY VOLUME DESCRIPTOR FOR CONTACT INFORMATION.')
...
def internal_check_dotdot_dir_record(dotdot_record, rr=False, rr_nlinks=0):
    ...
    if rr:
        ...
        assert(dotdot_record.rock_ridge.ext_id == None)
        assert(dotdot_record.rock_ridge.ext_des == None)
        assert(dotdot_record.rock_ridge.ext_src == None)
        assert(dotdot_record.rock_ridge.extension_sequence == None)
        assert(dotdot_record.rock_ridge.posix_name == "")
        assert(dotdot_record.rock_ridge.dev_t_high == None)
        assert(dotdot_record.rock_ridge.dev_t_low == None)
        assert(dotdot_record.rock_ridge.creation_time == None)
        assert(type(dotdot_record.rock_ridge.access_time) == pyiso.DirectoryRecordDate)
        assert(type(dotdot_record.rock_ridge.modification_time) == pyiso.DirectoryRecordDate)
        assert(type(dotdot_record.rock_ridge.attribute_change_time) == pyiso.DirectoryRecordDate)
        assert(dotdot_record.rock_ridge.backup_time == None)
        assert(dotdot_record.rock_ridge.expiration_time == None)
        assert(dotdot_record.rock_ridge.effective_time == None)
...
def check_rr_symlink(iso, filesize):
    ...
    assert(sym_dir_record.rock_ridge.posix_user_id == 0)
    assert(sym_dir_record.rock_ridge.posix_group_id == 0)
    assert(sym_dir_record.rock_ridge.posix_serial_number == 0)
    assert(sym_dir_record.rock_ridge.creation_time == None)
    assert(type(sym_dir_record.rock_ridge.access_time) == pyiso.DirectoryRecordDate)
    assert(type(sym_dir_record.rock_ridge.modification_time) == pyiso.DirectoryRecordDate)
    assert(type(sym_dir_record.rock_ridge.attribute_change_time) == pyiso.DirectoryRecordDate)
    assert(sym_dir_record.rock_ridge.backup_time == None)
    assert(sym_dir_record.rock_ridge.expiration_time == None)
...
def check_rr_symlink_dot(iso, filesize):
    ...
    assert(sym_dir_record.rock_ridge.effective_time == None)
    assert(len(sym_dir_record.rock_ridge.symlink_components) == 1)
    assert(sym_dir_record.rock_ridge.symlink_components[0] == '.')
...
def check_rr_verylongname(iso, filesize):
    ...
    assert(foo_dir_record.rock_ridge.ce_record.continuation_entry.posix_file_mode == 0100444)
    assert(foo_dir_record.rock_ridge.ce_record.continuation_entry.posix_file_links == 1)
    assert(foo_dir_record.rock_ridge.ce_record.continuation_entry.posix_user_id == 0)
    assert(foo_dir_record.rock_ridge.ce_record.continuation_entry.posix_group_id == 0)
    assert(foo_dir_record.rock_ridge.ce_record.continuation_entry.posix_serial_number == 0)
    assert(foo_dir_record.rock_ridge.creation_time == None)
    assert(type(foo_dir_record.rock_ridge.ce_record.continuation_entry.access_time) == pyiso.DirectoryRecordDate)
    assert(type(foo_dir_record.rock_ridge.ce_record.continuation_entry.modification_time) == pyiso.DirectoryRecordDate)
    assert(type(foo_dir_record.rock_ridge.ce_record.continuation_entry.attribute_change_time) == pyiso.DirectoryRecordDate)
    assert(foo_dir_record.rock_ridge.backup_time == None)
    assert(foo_dir_record.rock_ridge.expiration_time == None)
    assert(foo_dir_record.rock_ridge.effective_time == None)
